...
function eth_get_chain_env {
    chain_var_name="${1}_${2}"
    echo ${!chain_var_name}
}

# Usage: [CHAIN_NAME]
function eth_wait_for_network_start {
...
function eth_start_chain {
    echo "Starting local chain..." | pretty_log_term $1
    
    rpc_port=$2
    chain_name=$(echo $1 | tr '[:lower:]' '[:upper:]')
    chain_id=$(eth_get_chain_env $chain_name CHAIN_ID)
    fork_url=$(eth_get_chain_env $chain_name FORK_URL)
    fork_block_num=$(eth_get_chain_env $chain_name FORK_BLOCK_NUM)
